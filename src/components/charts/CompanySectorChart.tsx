...
export default function CompanySectorChart({ data }: CompanySectorChartProps) {
  const locale = useLocale()
  const [selectedSector, setSelectedSector] = useState<string | null>(null)

  // Debug: Component'e gelen veriyi logla
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('CompanySectorChart data:', data)
      console.log('CompanySectorChart data length:', data?.length || 0)
      console.log('CompanySectorChart data type:', Array.isArray(data) ? 'array' : typeof data)
    }
  }, [data])
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex items-center justify-center h-[300px] text-gray-500">
        <p>Henüz müşteri firma sektör dağılım verisi yok</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(value: number, name: string, props: any) => [
              `${value} firma (${props.payload.percent.toFixed(1)}%)`,
              'Firma Sayısı',
            ]}
      ...
      {/* Sektör Listesi ve Firma Detayları */}
      <div className="space-y-3">
        ...
        {/* Seçili Sektörün Firmaları */}
        {selectedData && selectedData.companies && selectedData.companies.length > 0 && (
          <Card className="p-4 border-2 border-primary-200 bg-primary-50/50">
            <div className="flex items-center gap-2 mb-3">
              ...
              <Badge variant="secondary" className="ml-auto">
                {selectedData.totalCompanies || selectedData.value} firma
              </Badge>
            </div>
            <div className="max-h-40 overflow-y-auto space-y-2">
              {selectedData.companies.map((company) => (
                <Link
                  key={company.id}
                  href={`/${locale}/customer-companies/${company.id}`}
                  prefetch={true}
              ...
              {selectedData.totalCompanies && selectedData.totalCompanies > selectedData.companies.length && (
                <div className="text-xs text-gray-500 text-center pt-2 border-t">
                  +{selectedData.totalCompanies - selectedData.companies.length} firma daha...
                </div>
              )}
            </div>
          </Card>
