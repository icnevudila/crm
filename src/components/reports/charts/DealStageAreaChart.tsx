...
export default function DealStageAreaChart({ data }: DealStageAreaChartProps) {
  ...
  // İlk 5 aşamayı al ve Türkçeleştir
    ...
    <div>
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
          ...
          <Tooltip
            ...
            labelFormatter={(label) => `${label} Aşaması`}
          />
          <Legend />
          <Area
            type="monotone"
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {chartData.map((item) => (
          <Link
            key={item.stage}
            href={`/${locale}/deals?stage=${item.stage}`}
            className="text-xs text-primary-600 hover:text-primary-700 hover:underline"
          >
            {item.stageLabel}: {item.count} fırsat →
          </Link>
        ))}
      </div>
    </div>
