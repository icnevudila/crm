...
export async function GET(request: Request) {
  try {
    ...
    // Outlook hem Calendar hem Email için kullanılabilir, state'ten tip belirlenebilir
    // Şimdilik MICROSOFT_EMAIL olarak kaydediyoruz
    const { data: existingUserIntegration } = await supabase
      .from('UserIntegration')
      .select('id')
      .eq('userId', stateData.userId)
      .eq('companyId', stateData.companyId)
      .eq('integrationType', 'MICROSOFT_EMAIL')
      .maybeSingle()
    ...
    if (existingUserIntegration) {
      ...
      await supabase
        ...
        .insert({
          ...
          companyId: stateData.companyId,
          integrationType: 'MICROSOFT_EMAIL',
          accessToken: access_token,
          refreshToken: refresh_token || null,
          tokenExpiresAt: expiresAt,
          status: 'ACTIVE',
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/user-integrations?success=outlook_connected`
    )
  } catch (error: any) {
