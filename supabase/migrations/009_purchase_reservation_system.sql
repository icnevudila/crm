-- Purchase Reservation System Migration
-- Alış faturası kesildiğinde stok hemen artmasın, incomingQuantity (beklenen giriş) sistemi ile beklemeye alınsın
-- Alış işlemi onaylandığında stok artışı yapılsın

-- 1. Product tablosuna incomingQuantity (beklenen giriş) alanı ekle
ALTER TABLE "Product" 
ADD COLUMN IF NOT EXISTS "incomingQuantity" DECIMAL(10, 2) DEFAULT 0;

-- 2. PurchaseTransaction (alis_islemi) tablosu oluştur
...
-- 3. Invoice tablosuna purchaseShipmentId (alis_islemi_id) alanı ekle
ALTER TABLE "Invoice" 
...
-- 6. InvoiceItem silindiğinde incomingQuantity geri ekle (trigger)
...
-- 7. Alış işlemi onaylandığında stok artışı ve incomingQuantity azalması (trigger)
